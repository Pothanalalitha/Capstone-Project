...
public class PatientServiceImpl implements PatientService {
    ...
    public PatientResponseDTO addPatient(PatientRequestDTO dto) {
		...
		/*
		 * List<MedicalRecordDocument> patientMedRec = new
		 * ArrayList<MedicalRecordDocument>(); if (dto.getMedicalRecords() != null) {
		 * List<MedicalRecordDocument> records = dto.getMedicalRecords().stream()
		 * .map(recordDto -> { recordDto.setPatientId(patient.getId()); return
		 * MedicalRecordMapper.toDocument(recordDto); }) .collect(Collectors.toList());
		 * patientMedRec = medicalRecordMongoRepository.saveAll(records); }
		 * PatientResponseDTO patientRes = PatientMapper.toDTO(patient);
		 * if(patientMedRec!=null) patientRes.setMedicalRecords(patientMedRec);
		 */
        PatientResponseDTO patientRes = PatientMapper.toDTO(patient);
        return patientRes;
    }
    ...
    public PatientResponseDTO updatePatient(Long id, PatientRequestDTO dto) {
        ...
        List<MedicalRecordDTO> medicalRecords = medicalRecordDocs.stream()
                .map(MedicalRecordMapper::toDTO)
                .collect(Collectors.toList());
        responseDto.setMedicalRecords(medicalRecords);
